...
class SeparableConv2d(nn.Module):
    # 설명은 mobilenet에 적어났음음
    def __init__(self, in_channels, out_channels, kernel_size, **kwargs):

        super().__init__()
        self.depthwise = nn.Conv2d(
            in_channels, in_channels, kernel_size, groups=in_channels, bias=False, **kwargs
        )
...
class MiddleFlowBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        residual = self.comv3(residual)

        shortcut = self.shortcut(x)

        return residual + shortcut

class MiddleFlow(nn.Module): # 8번 반복
    def __init__(self, block):
        super().__init__()

        self.middle_block = self._make_flow(block, 8)
